...
public final class DefaultMovieSession {
    ...
    var apiKey = APIConstants.API_KEY
...
extension DefaultMovieSession: MovieSession {
    public func fetchMovies() async throws -> APIMovieTMDB {
        ...
        let request = HTTPRequestBuilder(baseURL: baseURL)
            .set(path: "discover/movie?api_key=\(String(describing: apiKey))&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate")
            .build()
